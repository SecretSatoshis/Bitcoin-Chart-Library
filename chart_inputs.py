...
def load_chart_inputs(csv_path, now=None):
    ...
    if manifest.get('version') != 1 or set(manifest.get('files', {})) != set(INPUT_FILES):
        raise ValueError('Missing or unsupported chart input manifest')
    frames = {}
    for filename in INPUT_FILES:
        payload = _read_bytes(csv_path(filename))
        if hashlib.sha256(payload).hexdigest() != manifest['files'][filename]:
    ...
    validate_report_dates(frames[INPUT_FILES[0]], frames['report_ohlc_summary.csv'], manifest['report_date'], now)
    return frames
